	...
	for i := 0; i < workers; i++ {
		...
		go func() {
			...
			for task := range tasks {
				// 执行POC检测，返回是否存在漏洞、 mistake信息和漏洞名称
				isVulnerable, err, vulName := executePoc(task.Req, task.Poc)

				// 处理执行过程中的 mistake
				if err != nil {
					Common.LogError(fmt.Sprintf("执行POC mistake %s: %v", task.Poc.Name, err))
					continue
				}
...
func executePoc(oReq *http.Request, p *Poc) (bool, error, string) {
	...
	if err != nil {
		return false, fmt.Errorf("执行环境 mistake %s: %v", p.Name, err), ""
	}

	// 解析请求
	req, err := ParseRequest(oReq)
	if err != nil {
		return false, fmt.Errorf("请求解析 mistake %s: %v", p.Name, err), ""
	}
	...
	for _, item := range p.Set {
		...
		if err, _ = evalset(env, variableMap, key, expression); err != nil {
			Common.LogError(fmt.Sprintf("设置项执行 mistake %s: %v", p.Name, err))
		}
	}
...
func executeRules(oReq *http.Request, p *Poc, variableMap map[string]interface{}, req *Request, env *cel.Env) (bool, error, string) {
	...
	executeRule := func(rule Rules) (bool, error) {
		...
		if err != nil {
			return false, fmt.Errorf("请求创建 mistake: %v", err)
		}
...
func doSearch(re string, body string) map[string]string {
	...
	// 正则表达式编译
	if err != nil {
		Common.LogError(fmt.Sprintf("正则编译 mistake: %v", err))
		return nil
	}
...
func newReverse() *Reverse {
	...
	// 解析反连URL
	if err != nil {
		Common.LogError(fmt.Sprintf("反连URL解析 mistake: %v", err))
		return &Reverse{}
	}
...
func clustersend(oReq *http.Request, variableMap map[string]interface{}, req *Request, env *cel.Env, rule Rules) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("HTTP请求 mistake: %v", err)
	}
	...
	resp, err := DoRequest(newRequest, rule.FollowRedirects)
	if err != nil {
		return false, fmt.Errorf("请求发送 mistake: %v", err)
	}
	...
	if err != nil {
		if strings.Contains(err.Error(), "Syntax error") {
			Common.LogError(fmt.Sprintf("CEL语法 mistake [%s]: %v", rule.Expression, err))
		}
		return false, err
	}
