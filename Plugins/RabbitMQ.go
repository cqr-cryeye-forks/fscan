...
type RabbitMQScanResult struct {
	...
	ErrorMsg   string // 保存详细的 mistake信息
}
...
func RabbitMQScan(info *Common.HostInfo) error {
	...
	if defaultResult.Success {
		...
		// 打印默认账号的详细 mistake信息
		Common.LogDebug(fmt.Sprintf("默认账号 guest/guest 失败，详细 mistake: %s", defaultResult.ErrorMsg))
	}
...
func tryRabbitMQCredential(ctx context.Context, info *Common.HostInfo, credential RabbitMQCredential, timeoutSeconds int64, maxRetries int) *RabbitMQScanResult {
	...
	for retry := 0; retry < maxRetries; retry++ {
		...
		default:
			...
			// 打印详细的 mistake信息，包括所有原始 mistake信息
			Common.LogDebug(fmt.Sprintf("凭据 %s:%s 失败， mistake详情: %s",
				credential.Username, credential.Password, errorMsg))

			if err != nil {
				// 可以根据 mistake信息类型来决定是否需要重试
				// 例如，如果 mistake是认证 mistake，则无需重试
				if strings.Contains(errorMsg, "ACCESS_REFUSED") {
					Common.LogDebug("认证 mistake，无需重试")
					break
				}

				// 检查是否需要重试
				if retryErr := Common.CheckErrs(err); retryErr == nil {
					break // 不需要重试的 mistake
				}
			}
		}
	}
