	...
	for _, user := range common.Userdict["smb"] {
		for _, pass := range common.Passwords {
			...
			if flag == true && err == nil {
				var result string
				if common.Domain != "" {
					result = fmt.Sprintf("[+] %v:%v SMB:%v user:%v password:%v", info.Host, info.Ports, common.Domain, user, pass)
				} else {
					result = fmt.Sprintf("[+] %v:%v SMB user:%v password:%v", info.Host, info.Ports, user, pass)
				}
