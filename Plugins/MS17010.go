...
func init() {
	...
	if err != nil {
		Common.LogError(fmt.Sprintf("协议请求解密 mistake: %v", err))
		os.Exit(1)
	}
	negotiateProtocolRequest, err = hex.DecodeString(decrypted)
	if err != nil {
		Common.LogError(fmt.Sprintf("协议请求解码 mistake: %v", err))
		os.Exit(1)
	}
	...
	if err != nil {
		Common.LogError(fmt.Sprintf("会话请求解密 mistake: %v", err))
		os.Exit(1)
	}
	sessionSetupRequest, err = hex.DecodeString(decrypted)
	if err != nil {
		Common.LogError(fmt.Sprintf("会话请求解码 mistake: %v", err))
		os.Exit(1)
	}
	...
	if err != nil {
		Common.LogError(fmt.Sprintf("连接请求解密 mistake: %v", err))
		os.Exit(1)
	}
	treeConnectRequest, err = hex.DecodeString(decrypted)
	if err != nil {
		Common.LogError(fmt.Sprintf("连接请求解码 mistake: %v", err))
		os.Exit(1)
	}
	...
	if err != nil {
		Common.LogError(fmt.Sprintf("管道请求解密 mistake: %v", err))
		os.Exit(1)
	}
	transNamedPipeRequest, err = hex.DecodeString(decrypted)
	if err != nil {
		Common.LogError(fmt.Sprintf("管道请求解码 mistake: %v", err))
		os.Exit(1)
	}
	...
	if err != nil {
		Common.LogError(fmt.Sprintf("会话设置解密 mistake: %v", err))
		os.Exit(1)
	}
	trans2SessionSetupRequest, err = hex.DecodeString(decrypted)
	if err != nil {
		Common.LogError(fmt.Sprintf("会话设置解码 mistake: %v", err))
		os.Exit(1)
	}
}
...
func MS17010Scan(info *Common.HostInfo) error {
	...
	if _, err = conn.Write(sessionSetupRequest); err != nil {
		return fmt.Errorf("发送会话请求 mistake: %v", err)
	}

	n, err := conn.Read(reply)
	if err != nil || n < 36 {
		if err != nil {
			return fmt.Errorf("读取会话响应 mistake: %v", err)
		}
	...
	if _, err = conn.Write(treeConnectRequest); err != nil {
		return fmt.Errorf("发送树连接请求 mistake: %v", err)
	}

	if n, err := conn.Read(reply); err != nil || n < 36 {
		if err != nil {
			return fmt.Errorf("读取树连接响应 mistake: %v", err)
		}
	...
	if _, err = conn.Write(transNamedPipeRequest); err != nil {
		return fmt.Errorf("发送管道请求 mistake: %v", err)
	}

	if n, err := conn.Read(reply); err != nil || n < 36 {
		if err != nil {
			return fmt.Errorf("读取管道响应 mistake: %v", err)
		}
	...
	if reply[9] == 0x05 && reply[10] == 0x02 && reply[11] == 0x00 && reply[12] == 0xc0 {
		...
		if _, err = conn.Write(trans2SessionSetupRequest); err != nil {
			return fmt.Errorf("发送后门检测请求 mistake: %v", err)
		}

		if n, err := conn.Read(reply); err != nil || n < 36 {
			if err != nil {
				return fmt.Errorf("读取后门检测响应 mistake: %v", err)
			}
