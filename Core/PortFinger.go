...
func init() {
	...
	if nullProbe, ok := v.ProbesMapKName["NULL"]; ok {
		Common.LogDebug(fmt.Sprintf("Successfully obtained NULL detector, Data length: %d", len(nullProbe.Data)))
		null = &nullProbe
	} else {
		Common.LogDebug("WARNING: NULL detector not found")
	}

	// 获取并检查 GenericLines 探测器
	if commonProbe, ok := v.ProbesMapKName["GenericLines"]; ok {
		Common.LogDebug(fmt.Sprintf("Successfully obtained GenericLines detector, Data length: %d", len(commonProbe.Data)))
		common = &commonProbe
	} else {
		Common.LogDebug("WARNING: Generic Lines detector not found")
	}

	Common.LogDebug("Global variable initialization completed")
}
...
func (p *Probe) getDirectiveSyntax(data string) (directive Directive) {
	...
	Common.LogDebug(fmt.Sprintf("Command parsing result: name=%s, flag=%s, separator=%s, content=%s",
...
func (p *Probe) parseProbeInfo(probeStr string) {
	Common.LogDebug("Start parsing detector information, input string: " + probeStr)
	...
	if !(proto == "TCP " || proto == "UDP ") {
		...
		Common.LogDebug(" mistake: " + errMsg)
		panic(errMsg)
	}
	...
	Common.LogDebug(fmt.Sprintf("Detector parsing completed: Name=%s, Data=%s, Protocol=%s",
...
func (p *Probe) fromString(data string) error {
	...
	p.Matchs = &matchs
	Common.LogDebug(fmt.Sprintf("Parsing completed, there are %d matching rules", len(matchs)))
	return err
}
...
func (p *Probe) parseTCPWrappedMS(data string) {
	...
	Common.LogDebug(fmt.Sprintf("TCP wrapper wait time: %d ms", wrappedMS))
}
...
func (v *VScan) parseProbesFromContent(content string) {
	...
	if !(strings.HasPrefix(firstLine, "Exclude ") || strings.HasPrefix(firstLine, "Probe ")) {
		errMsg := "解析 mistake: 首行必须以\"Probe \"或\"Exclude \"开头"
		Common.LogDebug(" mistake: " + errMsg)
		panic(errMsg)
	}
	...
	for _, probePart := range probeParts {
		...
		if err := probe.fromString(probePart); err != nil {
			Common.LogDebug(fmt.Sprintf("Parsing probe failed: %v", err))
			continue
		}
...
// Init 初始化VScan对象
func (v *VScan) Init() {
	Common.LogDebug("Start initializing VScan")
