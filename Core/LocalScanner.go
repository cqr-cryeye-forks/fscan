...
func (s *LocalScanStrategy) LogPluginInfo() {
	...
	if isCustomMode {
		Common.LogBase(fmt.Sprintf("Local mode: Use specified plugin: %s", strings.Join(allPlugins, ", ")))
		return
	}
	...
	if len(applicablePlugins) > 0 {
		Common.LogBase(fmt.Sprintf("Local Mode: Use local plugins: %s", strings.Join(applicablePlugins, ", ")))
	} else {
		Common.LogBase("Local Mode: No available local plugins found")
	}
}
